export default function Skills() {
  ...
  return (
    <section id="skills" className="flex flex-col gap-8">
      <div>
        <h3 className="text-green-700 dark:text-primary font-semibold tracking-wide text-sm mb-2"><span className="text-rose-600 dark:text-pink-500">const</span> <span className="text-blue-600 dark:text-blue-400">technicalArsenal</span> = [</h3>
        <h2 className="text-3xl md:text-4xl font-bold text-gray-900 dark:text-green-100">&gt;_ Skills & Technologies</h2>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 pl-4 border-l border-green-600/30 dark:border-primary/30 ml-2">
          ...
          <div key={idx} className="code-card flex flex-col gap-4 hover:bg-green-50 dark:hover:bg-primary/5 transition-colors">
            ...
            <ul className="flex flex-col gap-2">
              {category.skills.map((skill, sIdx) => (
                <li key={sIdx} className="flex items-center gap-2 text-gray-700 dark:text-green-200">
                  <span className="text-green-700 dark:text-primary text-xs opacity-50">&gt;</span> 
                  "{skill}"
                </li>
              ))}
            </ul>
