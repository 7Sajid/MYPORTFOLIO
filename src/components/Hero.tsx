...
export default function Hero() {
  ...
  return (
    <section id="hero" className="min-h-[80vh] flex flex-col justify-center gap-6 relative">
      ...
      <div className="z-10">
        <div className="text-primary dark:text-primary font-mono mb-4 flex items-center gap-2">
          <span className="text-gray-500 dark:text-gray-400">~/portfolio</span> $ ./run_profile.sh
        </div>
        <h2 className="text-[5.5vw] sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold text-secondary dark:text-secondary drop-shadow-[0_0_15px_rgba(255,211,61,0.5)] tracking-tight leading-tight mb-2 whitespace-nowrap overflow-visible">
          HELLO I AM RAFAT UL AMAN SAJID
        </h2>
        <div className="text-sm sm:text-lg md:text-2xl lg:text-3xl font-mono text-gray-800 dark:text-primary font-semibold mb-6 flex flex-wrap items-center gap-x-2 gap-y-1 min-h-[4rem] sm:min-h-[3rem] md:min-h-10">
          <span className="text-rose-600 dark:text-pink-500">const</span> 
          <span className="text-blue-600 dark:text-blue-400">role</span> 
          <span>=</span> 
          <span className="text-secondary dark:text-secondary break-words">"{typedText}"</span>
          <span className="animate-pulse text-primary dark:text-primary -ml-1">|</span>
        </div>

        <p className="text-gray-600 dark:text-primary/70 text-lg md:text-xl max-w-2xl font-mono">
